...
contract('Timelock', ([alice, bob, carol, dev, minter]) => {
    beforeEach(async () => {
        this.nrfx = await NarfexToken.new({ from: alice });
        this.timelock = await Timelock.new(bob, '28800', { from: alice }); //8hours
    });

    it('should not allow non-owner to do operation', async () => {
        await this.nrfx.transferOwnership(this.timelock.address, { from: alice });
        await expectRevert(
            this.nrfx.transferOwnership(carol, { from: alice }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.nrfx.transferOwnership(carol, { from: bob }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.timelock.queueTransaction(
                this.nrfx.address, '0', 'transferOwnership(address)',
    ...
    it('should do the timelock thing', async () => {
        ...
        assert.equal((await this.nrfx.owner()).valueOf(), carol);
    });
    ...
    it('should also work with MasterChef', async () => {
        ...
        this.chef = await MasterChef.new(this.nrfx.address, this.syrup.address, dev, '1000', '0', { from: alice });
        await this.nrfx.transferOwnership(this.chef.address, { from: alice });
